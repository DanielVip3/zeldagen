...
def crossover(parent1, parent2):
    ...
    child1_leaves = [node for node, attr in child1.nodes(data=True) if child1.out_degree(node) == 0 and attr.get('type') != Types.FINISH]
    child2_leaves = [node for node, attr in child2.nodes(data=True) if child2.out_degree(node) == 0 and attr.get('type') != Types.FINISH]
    ...
    leaf1_predecessors = list(child1.predecessors(random_leaf1))
    leaf2_predecessors = list(child2.predecessors(random_leaf2))
    ...
    leaf1_type = child1.nodes[random_leaf1]['type']
    leaf2_type = child2.nodes[random_leaf2]['type']

    child1.nodes[random_leaf1]['type'] = leaf2_type
    child2.nodes[random_leaf2]['type'] = leaf1_type

    edge1_locked = child1.edges[random_leaf1_predecessor, random_leaf1]['locked']
    edge2_locked = child2.edges[random_leaf2_predecessor, random_leaf2]['locked']

    child1.edges[random_leaf1_predecessor, random_leaf1]['locked'] = edge2_locked
    child2.edges[random_leaf2_predecessor, random_leaf2]['locked'] = edge1_locked

    return child1, child2

def should_lock(individual):
    locked_edges = [(node1, node2) for node1, node2, attr in individual.edges(data=True) if attr.get('locked')]
...
def mutation(individual):
    while True:
        ...
        if action == 1: # add an edge
            if len(individual.edges) >= (NUM_ROOMS + (NUM_ROOMS // 3)):
                continue

            node = random.choice(range(ROOT, FINISH))

            possible_connections = [poss_node for poss_node, attr in individual.nodes(data=True)
                                    if attr.get('type') != Types.FINISH
                                    and node != poss_node
                                    and not individual.has_edge(poss_node, node)
                                    and not individual.has_edge(node, poss_node)]
            if not possible_connections:
                continue

            node_to_connect = random.choice(possible_connections)

            individual.add_edge(node, node_to_connect, locked=should_lock(individual))
        elif action == 2: # remove an edge
            if len(individual.edges) < (NUM_ROOMS + (NUM_ROOMS // 3)):
                continue

            bridges = list(nx.bridges(individual.to_undirected()))

            possible_edges = [(node1, node2) for node1, node2, attr in individual.edges(data=True)
                              if individual.nodes[node1]['type'] != Types.START
                              and individual.nodes[node2]['type'] != Types.FINISH
            ...
            individual.remove_edge(*edge)
        elif action == 3: # change node type
            node = random.choice(range(ROOT + 1, FINISH))
            individual.nodes[node]['type'] = random.choice([Types.NORMAL, Types.KEY])
        elif action == 4: # toggle edge lock
            to_lock = should_lock(individual)

            possible_edges = [(node1, node2) for node1, node2, attr in individual.edges(data=True)
                              if individual.nodes[node2]['type'] != Types.FINISH and attr.get('locked') != to_lock]
            if not possible_edges:
                continue

            edge = random.choice(possible_edges)
            individual.edges[edge]['locked'] = to_lock
        break
